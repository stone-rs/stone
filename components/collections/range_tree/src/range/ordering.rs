...
use range_traits::{PartialEnum, MaybeBounded};
use std::{
    ...
    U: PartialEnum,
{
    ...
    match (b1, b2) {
        ...
        (Bound::Included(v1), Bound::Unbounded) => Some(BoundOrdering::Included(
            (start && *v1 == U::min().unwrap()) || (!start && *v1 == U::max().unwrap()),
        )),
        ...
        (Bound::Unbounded, Bound::Included(v2)) => {
            if (start && T::min().unwrap() == *v2) || (!start && T::max().unwrap() == *v2) {
                ...
                Some(BoundOrdering::Excluded(
                    (start
                        && v2
                            .pred()
                            .map(|pred| T::min().unwrap() == pred)
                            .unwrap_or(false))
                        || (!start
                            && v2
                                .succ()
                                .map(|succ| T::min().unwrap() == succ)
                                .unwrap_or(false)),
                ))
            }
        }
    ...
    T: Measure + PartialEnum + Ord,
{
    ...
    match (b1, b2) {
        ...
        (Bound::Included(v1), Bound::Unbounded) => BoundOrdering::Included(
            (start && *v1 == <T as MaybeBounded>::min().unwrap()) || (!start && *v1 == <T as MaybeBounded>::max().unwrap()),
        ),
        ...
        (Bound::Unbounded, Bound::Included(v2)) => {
            if (start && <T as MaybeBounded>::min().unwrap() == *v2) || (!start && <T as MaybeBounded>::max().unwrap() == *v2) {
                ...
                BoundOrdering::Excluded(
                    (start
                        && v2
                            .pred()
                            .map(|pred| <T as MaybeBounded>::min().unwrap() == pred)
                            .unwrap_or(false))
                        || (!start
                            && v2
                                .succ()
                                .map(|succ| <T as MaybeBounded>::min().unwrap() == succ)
                                .unwrap_or(false)),
                )
            }
        }
    ...
    U: PartialEnum,
{
    ...
    match (b1, b2) {
        ...
        (Bound::Excluded(v1), Bound::Unbounded) => {
            if (!b2_start && *v1 == U::min().unwrap()) || (b2_start && *v1 == U::max().unwrap()) {
                ...
                Some(BoundOrdering::Included(
                    (!b2_start
                        && v1
                            .pred()
                            .map(|pred| pred == U::min().unwrap())
                            .unwrap_or(false))
                        || (b2_start
                            && v1
                                .succ()
                                .map(|succ| succ == U::max().unwrap())
                                .unwrap_or(false)),
                ))
            }
        }
        ...
        (Bound::Unbounded, Bound::Excluded(v2)) => {
            if (!b2_start && T::min().unwrap() == *v2) || (b2_start && T::max().unwrap() == *v2) {
                Some(BoundOrdering::Excluded(true))
            } else {
                Some(BoundOrdering::Included(
                    (!b2_start
